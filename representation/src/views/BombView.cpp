...
sf::IntRect bombRect(int left, int top) { return {left, top, kBombFrameWidth, kBombFrameHeight}; }
...
BombAnimationSet makeBombAnimationSet() {
    const std::array<sf::IntRect, 4> bombFrames{bombRect(103, 117), bombRect(120, 117), bombRect(137, 117),
                                                bombRect(120, 117)};

    const std::array<sf::IntRect, 9> center{explosionRect(69, 83),  explosionRect(120, 66), explosionRect(103, 66),
                                            explosionRect(86, 66),  explosionRect(69, 66),  explosionRect(86, 66),
                                            explosionRect(103, 66), explosionRect(120, 66), explosionRect(69, 83)};
    const std::array<sf::IntRect, 9> endUp{explosionRect(69, 49),  explosionRect(86, 49),  explosionRect(103, 49),
    ...
    const std::array<sf::IntRect, 9> endDown{explosionRect(52, 49),   explosionRect(69, 100), explosionRect(120, 100),
    ...
    const std::array<sf::IntRect, 9> endLeft{explosionRect(1, 32),  explosionRect(18, 32), explosionRect(35, 32),
                                             explosionRect(52, 32), explosionRect(69, 32), explosionRect(52, 32),
                                             explosionRect(35, 32), explosionRect(18, 32), explosionRect(1, 32)};
    const std::array<sf::IntRect, 9> endRight{explosionRect(52, 83),  explosionRect(52, 66), explosionRect(18, 100),
    ...
    const std::array<sf::IntRect, 9> bodyHorizontal{
        explosionRect(35, 83), explosionRect(35, 66), explosionRect(1, 100), explosionRect(1, 83), explosionRect(1, 66),
        explosionRect(1, 83),  explosionRect(1, 100), explosionRect(35, 66), explosionRect(35, 83)};
...
void BombView::drawFrame(sf::RenderWindow& window, const Camera& camera, const sf::IntRect& frame,
    ...
    sprite.setScale(screenSize.x / static_cast<float>(frame.width), screenSize.y / static_cast<float>(frame.height));
    window.draw(sprite);
}

void BombView::draw(sf::RenderWindow& window, const Camera& camera) {
    if (!model || markedForRemoval || phase == Phase::Done)
        return;
    ...
    const auto drawDirection = [&](logic::Direction direction, const std::array<sf::IntRect, 9>& bodyFrames,

...
sf::IntRect rect(int left, int top) { return {left, top, kWallFrameWidth, kWallFrameHeight}; }
...
WallAnimationSet makeWallAnimationSet(WallSpriteVariant variant) {
    ...
    case WallSpriteVariant::Destructible:
        return makeSet({rect(35, 15)},
                       {rect(1, 117), rect(18, 117), rect(35, 117), rect(52, 117), rect(69, 117), rect(86, 117)});
    }
    return makeSet({rect(18, 15)}, {});
}

WallView::WallView(std::shared_ptr<const Wall> model, std::shared_ptr<sf::Texture> texture,
...
void WallView::draw(sf::RenderWindow& window, const Camera& camera) {
    ...
    sprite.setTextureRect(
        breaking ? animationSet.breaking[std::min(
                       static_cast<std::size_t>(breakTimer / (kBreakDuration / animationSet.breaking.size())),
                       animationSet.breaking.size() - 1)]
                 : animationSet.idle[0]);
